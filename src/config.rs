...
pub fn parse_and_validate(config_json: &str) -> Result<RuntimeConfig, String> {
    ...
    let feed_url = Url::parse(config.feed_url.trim())
        .map_err(|error| format!("feed_url 不是有效 URL：{error}"))?;
    ...
    let timezone = Tz::from_str(config.timezone.trim())
        .map_err(|_| format!("timezone 不是有效 IANA 时区：{}", config.timezone.trim()))?;

    let mut keys = BTreeSet::new();
    let mut targets = Vec::with_capacity(config.targets.len());
    for (index, target) in config.targets.into_iter().enumerate() {
        let name = required_trimmed(&target.name, &format!("targets[{index}].name"))?;
        let account_id =
            required_trimmed(&target.account_id, &format!("targets[{index}].account_id"))?;
        let group_id = required_trimmed(&target.group_id, &format!("targets[{index}].group_id"))?;
