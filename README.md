...
- 后台线程在动态库卸载前停止并 `join`。
- 管理员命令 `ainews status` 查看运行状态，`ainews push` 立即检查最新一期。
...
`/ainews status` 只读取内存快照，不访问 RSS、不下载图片、不发送测试消息。输出包括插件/API/配置版本、worker 状态、目标数量、最近轮询耗时、期号 ID 哈希前缀和最多 8 个目标的最近入队状态。每个目标结果同时包含脱敏账号、脱敏群标识、Host API 状态和本轮记录时间。

`/ainews push` 向现有后台 worker 投递一次立即检查请求，推送配置时区当天的最新一期。命令回调不会直接执行网络请求；实际结果稍后通过 `/ainews status` 和日志查看。普通模式保持逐目标去重，已入队目标会跳过。

`/ainews push --force` 只对这一次检查绕过去重，可能向所有启用目标重复发送当天最新一期。连续请求会合并为一次等待任务，强制请求优先于普通请求。插件关闭、没有启用目标、worker 未运行或处于状态保护模式时会拒绝手动推送。
